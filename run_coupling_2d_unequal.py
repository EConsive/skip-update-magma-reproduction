"""
2D coupling experiment, Move 1 — unequal diagonals.

Extends run_coupling_2d.py to the regime where the off-diagonal coupling
exceeds the soft coordinate's self-curvature.

Setup:
  H = [[a, c], [c, b]]  with  a=1, b=100  fixed.
  PSD requires c <= sqrt(a*b) = 10. We sweep c in {0, 1, 3, 6, 9, 9.9}, so
  the "coupling/soft-self-curvature" ratio c/a runs from 0 up to ~10.

  At c >= 1 the off-diagonal already exceeds the soft diagonal a=1.
  At c = 9.9 the Hessian is nearly degenerate (smallest eigenvalue ~0.02,
  condition number ~5000).

  Same comparison as the original: SGD+Momentum vs SkipUpdate(SGD+Mom, elem).
  Same isotropic Gaussian gradient noise (sigma=1.0). Same number of seeds,
  iterations, fine LR grid, best-vs-best methodology.

Init:
  W_INIT = [1, 1] (not [1, 0] like the original) because the eigenvectors of
  H are now near-axis-aligned (since b >> a), so [1, 0] would put almost all
  mass on the slow eigenvector. [1, 1] has nontrivial projection on both,
  regardless of how the off-diagonal rotates the eigenstructure.

Hypothesis being probed:
  The original "coupling helps SkipUpdate" hypothesis predicts that as c/a
  grows (i.e. coupling dominates the soft coordinate's self-curvature),
  SkipUpdate's masking gives an increasing advantage — because the masked
  update on the soft coordinate is now mostly driven by coupling-from-the-
  stiff-coordinate, not by its own gradient term.

  Falsified by an invariant ratio across c.
"""

import os
import time
import numpy as np
import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt

# Element-wise masking doesn't depend on BLOCK_SLICES, so no patching needed.
from optimizers import SGDMomentum, SkipUpdate
from run_experiments import LOG_EVERY, RESULTS_DIR

DIM = 2
A_DIAG = 1.0
B_DIAG = 100.0
COUPLINGS = [0.0, 1.0, 3.0, 6.0, 9.0, 9.9]   # c/sqrt(ab) up to 0.99
NOISE_SIGMA = 1.0
NUM_SEEDS = 30
N_ITERS = 10000
LRS = np.logspace(-5, -1, 25).tolist()       # 1e-5 to 0.1
W_INIT = np.array([1.0, 1.0])                 # projection on both eigenvectors


def make_H(c):
    return np.array([[A_DIAG, c], [c, B_DIAG]])


def eig_summary(c):
    H = make_H(c)
    lams = np.linalg.eigvalsh(H)
    return float(lams[0]), float(lams[-1])


def loss_fn(w, H):
    return 0.5 * w @ H @ w


def noisy_grad(w, H, rng, sigma):
    return H @ w + sigma * rng.standard_normal(len(w))


def ema_smooth(x, alpha=0.02):
    out = np.empty_like(x)
    out[0] = x[0]
    for i in range(1, len(x)):
        out[i] = alpha * x[i] + (1 - alpha) * out[i - 1] if np.isfinite(x[i]) else out[i - 1]
    return out


def run_one(optimizer, H, seed, sigma):
    rng = np.random.default_rng(seed * 10000 + 9999)
    w = W_INIT.copy()
    losses = []
    for t in range(N_ITERS):
        if t % LOG_EVERY == 0:
            losses.append(loss_fn(w, H))
        g = noisy_grad(w, H, rng, sigma)
        w = optimizer.step(w, g, rng)
        if not np.isfinite(w).all() or loss_fn(w, H) > 1e20:
            remaining = (N_ITERS - t - 1) // LOG_EVERY
            losses.extend([float("nan")] * remaining)
            break
    return np.array(losses)


def run_seeds(make_opt, H, sigma):
    all_losses = []
    for seed in range(NUM_SEEDS):
        losses = run_one(make_opt(), H, seed, sigma)
        all_losses.append(losses)
    max_len = max(len(l) for l in all_losses)
    padded = [np.concatenate([l, np.full(max_len - len(l), np.nan)])
              if len(l) < max_len else l for l in all_losses]
    return np.stack(padded)


def median_final(losses):
    finals = [l[np.isfinite(l)][-1] if np.any(np.isfinite(l)) else np.nan for l in losses]
    return np.nanmedian(finals)


def main():
    t0 = time.time()
    total = len(COUPLINGS) * len(LRS) * NUM_SEEDS * 2
    print(f"2D coupling experiment, Move 1 (unequal diagonals): {total} runs")
    print(f"  H = [[{A_DIAG}, c], [c, {B_DIAG}]],  c in {COUPLINGS}")
    print(f"  sigma={NOISE_SIGMA}, {NUM_SEEDS} seeds, {N_ITERS} iters, {len(LRS)} LRs")
    print(f"  w_init={W_INIT.tolist()}")

    results = {c: {"sgdmom": {}, "skipupdate": {}} for c in COUPLINGS}

    for c in COUPLINGS:
        H = make_H(c)
        lam_min, lam_max = eig_summary(c)
        cond = lam_max / max(lam_min, 1e-9)
        print(f"  c={c}: lambda=[{lam_min:.4f}, {lam_max:.4f}], "
              f"cond={cond:.1f}, c/a={c/A_DIAG:.2f}")
        for lr in LRS:
            sgd_opt = lambda _lr=lr: SGDMomentum(DIM, _lr)
            su_opt = lambda _lr=lr: SkipUpdate(SGDMomentum(DIM, _lr), masking="element")
            results[c]["sgdmom"][lr] = run_seeds(sgd_opt, H, NOISE_SIGMA)
            results[c]["skipupdate"][lr] = run_seeds(su_opt, H, NOISE_SIGMA)
        print(f"    done ({time.time() - t0:.0f}s)")

    print(f"\n{'=' * 84}")
    print(f"Best-vs-best comparison (median final loss, {NUM_SEEDS} seeds)")
    print(f"{'=' * 84}")
    print(f"{'c':>6} {'c/a':>6} {'cond':>10} {'SGD+Mom LR':>12} {'SGD+Mom':>14} "
          f"{'SkipU LR':>12} {'SkipU':>14} {'Ratio':>8}")
    print("-" * 84)

    best = {}
    for c in COUPLINGS:
        sgd_finals = {lr: median_final(results[c]["sgdmom"][lr]) for lr in LRS}
        su_finals = {lr: median_final(results[c]["skipupdate"][lr]) for lr in LRS}
        best_sgd_lr = min(sgd_finals, key=lambda k: sgd_finals[k] if np.isfinite(sgd_finals[k]) else np.inf)
        best_su_lr = min(su_finals, key=lambda k: su_finals[k] if np.isfinite(su_finals[k]) else np.inf)
        sgd_loss = sgd_finals[best_sgd_lr]
        su_loss = su_finals[best_su_lr]
        ratio = su_loss / sgd_loss if sgd_loss > 0 else np.inf
        winner = "SkipU" if ratio < 1.0 else "SGD+M"
        lam_min, lam_max = eig_summary(c)
        cond = lam_max / max(lam_min, 1e-9)
        print(f"{c:>6.2f} {c / A_DIAG:>6.2f} {cond:>10.1f} "
              f"{best_sgd_lr:>12.5f} {sgd_loss:>14.8f} "
              f"{best_su_lr:>12.5f} {su_loss:>14.8f} {ratio:>8.3f} {winner}")
        best[c] = {
            "sgd_lr": best_sgd_lr, "sgd_loss": sgd_loss,
            "su_lr": best_su_lr, "su_loss": su_loss,
            "ratio": ratio,
            "sgd_finals": sgd_finals, "su_finals": su_finals,
        }

    # Plot 1: LR sweep per coupling
    fig, axes = plt.subplots(1, len(COUPLINGS), figsize=(4 * len(COUPLINGS), 5),
                             sharey=True)
    for i, c in enumerate(COUPLINGS):
        ax = axes[i]
        lr_arr = np.array(LRS)
        sgd_arr = np.array([best[c]["sgd_finals"][lr] for lr in LRS])
        su_arr = np.array([best[c]["su_finals"][lr] for lr in LRS])
        ax.loglog(lr_arr, sgd_arr, "o-", color="#1f77b4",
                  label="SGD+Momentum", linewidth=2, markersize=5)
        ax.loglog(lr_arr, su_arr, "s-", color="#d62728",
                  label="SkipUpdate(SGD+Mom,elem)", linewidth=2, markersize=5)
        ax.axvline(best[c]["sgd_lr"], color="#1f77b4", linestyle=":", alpha=0.4)
        ax.axvline(best[c]["su_lr"], color="#d62728", linestyle=":", alpha=0.4)
        lam_min, lam_max = eig_summary(c)
        ax.set_title(f"c={c} (c/a={c / A_DIAG:.1f}, cond={lam_max / max(lam_min, 1e-9):.0f})",
                     fontsize=10)
        ax.set_xlabel("Learning Rate")
        if i == 0:
            ax.set_ylabel("Median Final Loss")
        ax.grid(True, alpha=0.3)
        if i == 0:
            ax.legend(fontsize=9)
    fig.suptitle(f"2D Coupling Move 1 (unequal diag, a={A_DIAG}, b={B_DIAG}): "
                 f"LR sweeps ({NUM_SEEDS} seeds, sigma={NOISE_SIGMA})", fontsize=12)
    plt.tight_layout()
    path = os.path.join(RESULTS_DIR, "coupling_2d_unequal_lr_sweep.png")
    fig.savefig(path, dpi=150)
    print(f"\nSaved: {path}")

    # Plot 2: Best-vs-best ratio as a function of coupling
    fig, ax = plt.subplots(figsize=(8, 5))
    c_arr = np.array(COUPLINGS)
    ratios = np.array([best[c]["ratio"] for c in COUPLINGS])
    ax.semilogy(c_arr, ratios, "o-", color="#8e44ad", linewidth=2, markersize=8)
    ax.axhline(1.0, color="gray", linestyle="--", alpha=0.6, label="parity")
    ax.set_xlabel(f"Off-diagonal coupling c  (a={A_DIAG}, b={B_DIAG})")
    ax.set_ylabel("SkipUpdate / SGD+Momentum (best-vs-best)")
    ax.set_title(f"Best-vs-best ratio vs coupling, unequal diagonals "
                 f"({NUM_SEEDS} seeds, sigma={NOISE_SIGMA})")
    ax.grid(True, alpha=0.3)
    ax.legend()
    for i, c in enumerate(COUPLINGS):
        ax.annotate(f"{ratios[i]:.2f}", (c_arr[i], ratios[i]),
                    textcoords="offset points", xytext=(8, 4), fontsize=9)
    plt.tight_layout()
    path = os.path.join(RESULTS_DIR, "coupling_2d_unequal_ratio.png")
    fig.savefig(path, dpi=150)
    print(f"Saved: {path}")

    # Plot 3: Convergence curves at best LRs
    fig, axes = plt.subplots(1, len(COUPLINGS), figsize=(4 * len(COUPLINGS), 5),
                             sharey=True)
    for i, c in enumerate(COUPLINGS):
        ax = axes[i]
        for name, key, lr_key, color in [
            ("SGD+Mom", "sgdmom", "sgd_lr", "#1f77b4"),
            ("SkipU(elem)", "skipupdate", "su_lr", "#d62728"),
        ]:
            lr = best[c][lr_key]
            losses = results[c][key][lr]
            iters = np.arange(losses.shape[1]) * LOG_EVERY
            med = ema_smooth(np.nanmedian(losses, axis=0))
            q25 = ema_smooth(np.nanpercentile(losses, 25, axis=0))
            q75 = ema_smooth(np.nanpercentile(losses, 75, axis=0))
            ax.semilogy(iters, med, label=f"{name} (lr={lr:.5f})",
                        color=color, linewidth=2)
            ax.fill_between(iters, q25, q75, alpha=0.15, color=color)
        ax.set_title(f"c={c}", fontsize=10)
        ax.set_xlabel("Gradient Steps")
        if i == 0:
            ax.set_ylabel("Loss (EMA-smoothed median)")
        ax.legend(fontsize=8)
        ax.grid(True, alpha=0.3)
    fig.suptitle(f"2D Coupling Move 1: convergence at best LRs ({NUM_SEEDS} seeds)",
                 fontsize=12)
    plt.tight_layout()
    path = os.path.join(RESULTS_DIR, "coupling_2d_unequal_curves.png")
    fig.savefig(path, dpi=150)
    print(f"Saved: {path}")
    print(f"Total: {time.time() - t0:.0f}s")


if __name__ == "__main__":
    main()
